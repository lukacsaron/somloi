...
function sm_plugins_loaded() {
	...
	if ( defined('SMPRO') && SMPRO === true ) {
		...
		//Code for handling Smart Manager Beta Access Privilege
		if ( ! class_exists( 'Smart_Manager_Pro_Access_Privilege' ) ) {
	        ...
	        if( !( ( !empty( $current_user->roles[0] ) && $current_user->roles[0] == 'administrator' ) || (!empty($current_user_caps) && $current_user_caps == 'administrator') ) ) {
	        	if (file_exists( (dirname( __FILE__ )) . '/new/pro/classes/class-smart-manager-pro-access-privilege.php' )) {
...
<?php
	...
	function sm_add_promo_notices() {
		...
		if ( !empty($_GET['page']) && ( 'smart-manager' === $_GET['page'] || 'smart-manager-woo' === $_GET['page'] || 'smart-manager-wpsc' === $_GET['page'] ) && false === $sm_dismiss_admin_notice ) {
			$sm_promo_msg = '';
	...
	function smart_woo_add_modules_admin_pages() {
        ...
        $position  = sm_get_free_menu_position(56.00001);

        $page = add_menu_page( 'Smart Manager', 'Smart Manager','read', 'smart-manager', 'sm_admin_page', 'dashicons-performance', $position );

        if( is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
        	add_submenu_page( 'edit.php?post_type=product', 'Smart Manager', 'Smart Manager', 'edit_pages', 'smart-manager-woo', 'sm_admin_page' );
        }
