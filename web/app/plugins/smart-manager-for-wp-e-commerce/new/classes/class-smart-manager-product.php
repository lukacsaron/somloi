...
if ( ! class_exists( 'Smart_Manager_Product' ) ) {
	class Smart_Manager_Product extends Smart_Manager_Base {
		...
		public function products_dashboard_model ($dashboard_model) {
			...
			$visible_columns = array('ID', '_thumbnail_id', 'post_title', '_sku', '_regular_price', '_sale_price', '_sale_price_dates_from', '_sale_price_dates_to', 
				...
				if (!empty($src)) {
					...
					if ((substr($src,0,3)=='pa_' && $col_table == 'terms') ) {
						...
						if ($src == 'product_cat') {
							...
							$column ['name_display']= 'Featured Image';
						}
					}
				}
			}
				...
				foreach ($data_model['items'] as $key => $data) {
					...
					if ( ! empty( $data['postmeta_meta_key__thumbnail_id_meta_value__thumbnail_id'] ) ) {
						...
						if ( is_array( $attachment ) && ! empty( $attachment[0] ) ) {
							if( !empty( $this->req_params['cmd'] ) && $this->req_params['cmd'] == 'get_export_csv' ) {
								$thumbnail = $attachment[0];
							} else {
								$thumbnail = '<img title="' . __( 'Change', Smart_Manager::$text_domain ) . '" src="' . $attachment[0] . '" width="20" height="20">';
							}
						} else {
							if( !empty( $this->req_params['cmd'] ) && $this->req_params['cmd'] == 'get_export_csv' ) {
								$thumbnail = '';
							} else {
								$thumbnail = '<div title="' . __( 'Set', Smart_Manager::$text_domain ) . '" width="20" height="20">&nbsp;</div>';
							}
						}
						...
						if( !empty( $this->req_params['cmd'] ) && $this->req_params['cmd'] == 'get_export_csv' ) {
							...
							$data_model['items'][$key]['postmeta_meta_key__thumbnail_id_meta_value__thumbnail_id'] = '<div title="' . __( 'Set', Smart_Manager::$text_domain ) . '" width="20" height="20">&nbsp;</div>';
						}
					}
		...
		public function inline_update_product_featured_image() {
		    ...
		    if ( ! empty( $_POST['update_field'] ) && 'postmeta_meta_key__thumbnail_id_meta_value__thumbnail_id' === $_POST['update_field'] ) {
		    	...
		    	if ( ! empty( $product_id ) && ! empty( $attachment_id ) ) {
		    		...
		    		echo esc_html__( 'Featured Image updated successfully', Smart_Manager::$text_domain );
		    	} else {
