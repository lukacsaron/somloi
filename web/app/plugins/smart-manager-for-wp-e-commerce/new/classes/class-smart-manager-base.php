...
if ( ! class_exists( 'Smart_Manager_Base' ) ) {
	class Smart_Manager_Base {
		...
		public function sm_query_join ($join, $wp_query_obj) {
			...
			if( !empty($this->req_params) && !empty($this->req_params['search_query']) && !empty($this->req_params['search_query'][0]) && strpos($join,'sm_advanced_search_temp') === false ) {
		...
		public function sm_query_post_where_cond ($where, $wp_query_obj) {
			...
			//Code for handling search
			if( !empty($this->req_params) && !empty($this->req_params['search_query']) && !empty($this->req_params['search_query'][0]) && strpos($where,'sm_advanced_search_temp.flag > 0') === false ) {
		...
		public function get_default_store_model() {
			...
			if ($posts_num_rows > 0) {
				foreach ($results_posts_col as $posts_col) {
					...
					$temp ['name'] = __(str_replace('Post', $this->dashboard_title, $temp ['name']), Smart_Manager::$text_domain);
                ...
                if (is_array($search_string_array)) {
                        ...
                        foreach ($search_string_array as $search_string) {
                            ...
                            if (!empty($search_string['table_name']) && $search_string['table_name'] == $wpdb->prefix.'posts') {
                                ...
                                } else if ( $search_data_type == "date" || $search_data_type == "datetime" ) {
                                	$posts_cond = $search_string['table_name'].".".$search_col . " ". $search_operator ." '" . $search_value ."' ";
                                } else {
		...
		public function get_data_model() {
			...
			if( $load_default_data_model ) { //condition to skip the default data model
		        ...
		        if ((!empty($this->req_params['search_query']) && !empty($this->req_params['search_query'][0]) && $this->req_params['search_query'][0] != '[]') || (!empty($this->req_params['searchText'])) ) {
		            ...
		            if (!empty($search_query_diff)) {
						...
						$this->process_search_cond(array( 'search_query' => (!empty($this->req_params['search_query'])) ? $this->req_params['search_query'] : '',
		        				...
		        				foreach ($postmeta_data as $postmeta_key => $postmeta_value) {
		        					...
		        					if (array_search($postmeta_key, $data_cols_serialized) !== false) {
										...
										if (!empty($postmeta_value[0])) {
											...
											$postmeta_comma_separated_val = array();

											foreach( $postmeta_value[0] as $key => $pm_value ) {
												if( is_numeric($key) && !is_array( $pm_value ) ) {
													$postmeta_comma_separated_val[] = $pm_value;
												} else {
													$postmeta_comma_separated_val = array();
													break;
												}
											...
											$postmeta_value[0] = ( !empty( $postmeta_comma_separated_val ) ) ? implode(",",$postmeta_comma_separated_val) : json_encode($postmeta_value[0]);	
										}
			        				}
			...
			$data_model = apply_filters('sm_data_model', $data_model, $data_cols, $data_cols_serialized);
						...
						if ( $update_table == 'posts' && $insert_post != 1 ) {
	    					...
	    					if (array_search($meta_key, $data_cols_serialized) !== false) {
								if (!empty($value)) {
									...
									if( empty( $value ) ) { // for comma separated string values
										$value = explode(",", $value);
									}
								}
	        				}
