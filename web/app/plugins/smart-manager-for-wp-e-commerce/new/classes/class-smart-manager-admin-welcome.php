...
class Smart_Manager_Admin_Welcome {
	...
	private function intro() {
		...
		<h2 class="nav-tab-wrapper">
			...
			<a class="nav-tab <?php if ( $_GET['landing-page'] == 'sm-beta' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( add_query_arg( array( 'landing-page' => 'sm-beta' ), $this->sm_redirect_url ) ); ?>">
				<?php _e( "Smart Manager", self::$text_domain ); ?><sup style="vertical-align: super;color:red;font-size:0.7em;font-weight:400;">New</sup>
			</a>
		</h2>
		<?php
	}
		...
		<?php $this->intro(); ?>
			...
			<div>
				<div class="headline-feature">
					<h2 style="text-align:center;"><?php _e( 'Introducing Smart Manager', self::$text_domain ); ?><sup style="vertical-align: super;color:red;font-size:0.5em;">New</sup></h2>
					<div class="featured-image" style="border:1px solid #0085ba;">
						<img src="<?php echo $this->plugin_url . '/../../images/smart-manager-new.png'?>" />
					</div>
					<p><?php echo __( 'Smart Manager', self::$text_domain ); ?> <sup style="vertical-align: super;color:red;">New</sup> <?php echo __('is nothing but a transformed more bigger version of the previous Smart Manager. It has ton’s of functionality and only promises to be better than what Smart Manager ever was.', self::$text_domain ); ?></p>
				</div>
				<center><h3><?php echo __( 'Digging Deeper into Smart Manager', self::$text_domain ); ?> <span style="color:red;">New</span>...</h3></center>
				<div class="feature-section col three-col" >
					<div>
						<h4><?php echo __( 'Everything Wordpress', self::$text_domain ); ?></h4>
						<p>
							<?php echo __( 'Unlike previous Smart Manager, Smart Manager', self::$text_domain ); ?> <sup style="vertical-align: super;color:red;">New</sup> <?php echo sprintf( __('gives you the power to manage %s and %s.', self::$text_domain ),'<code>all post types</code>', '<code>any custom field</code>'); ?>
						</p>
					</div>
					<div>
						<h4><?php echo __( 'Infinite Scrolling', self::$text_domain ); ?></h4>
						<p>
							<?php echo __( 'Unlike the older version of Smart Manager that displayed records in various pages, with Smart Manager', self::$text_domain ); ?> <sup style="vertical-align: super;color:red;">New</sup> <?php echo sprintf(__( 'Beta all the records are in one single page itself enabling %s and %s.', self::$text_domain ), '<code>one glance at all records</code>', '<code>faster loading</code>'); ?>
						</p>
					</div>
					<div class="last-feature">
						<h4><?php echo __( 'Improved Performance', self::$text_domain ); ?></h4>
						<p>
							<?php echo __( 'We\'ve worked on every inch of the coding to make Smart Manager', self::$text_domain ); ?><sup style="vertical-align: super;color:red;">New</sup> <?php echo __( 'Beta way faster and the performance a lot smoother.', self::$text_domain ); ?>
						</p>
					</div>
				</div>
				<a href="<?php echo $this->sm_redirect_url; ?>" class="button button-primary"><?php _e( 'Try Smart Manager New', self::$text_domain ); ?></a>
			</div>
		</div>
		<?php
	}
            ...
            <?php
            	$faqs = array(
            				...
            				array(
            						...
            						'ans' => sprintf(__( 'Currently, Smart Manager sorts only the records visible on a particular page and not on the entire database. However, we do have same functionality in the roadmap for Smart Manager%s and would be implemented soon.', self::$text_domain ), '<sup style="vertical-align: super;color:red;">New</sup>')
            					),
            				array(
	...
	public function smart_manager_welcome() {
		...
		wp_redirect( admin_url( 'admin.php?page=smart-manager&landing-page=sm-beta' ) );
