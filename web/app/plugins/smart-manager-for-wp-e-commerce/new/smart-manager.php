...
if ( ! class_exists( 'Smart_Manager' ) ) {
	class Smart_Manager {
		...
		static $text_domain, $prefix, $sku, $plugin_file, $sm_is_woo30, $sm_is_woo22, $sm_is_woo21;
		...
		function define_constants() {
			...
			$this->sm_dashboards_final ['post'] = __(ucwords('post'), self::$text_domain);
			$this->sm_dashboards_final ['page'] = __(ucwords('page'), self::$text_domain);
			$this->sm_dashboards_final ['user'] = __(ucwords('users'), self::$text_domain);
			...
			if (!empty($sm_dashboards)) {
				foreach ($sm_dashboards as $sm_dashboard) {
					...
					$this->sm_dashboards_final [$sm_dashboard] = (!empty($dashboard_names[$sm_dashboard])) ? $dashboard_names[$sm_dashboard] : __(ucwords(str_replace(array('_','-'), array(' ',' '), $sm_dashboard)), self::$text_domain);
				}	
			}
		...
		function enqueue_admin_scripts() {
			...
			$sm_dashboard_keys = array_keys($this->sm_dashboards_final);
	        ...
	        wp_localize_script( 'sm_custom_smart_manager_js', 'sm_beta_params', array('sm_dashboards' => json_encode($this->sm_dashboards_final), 'SM_IS_WOO30' => self::$sm_is_woo30, 'SM_IS_WOO22' => self::$sm_is_woo22, 'SM_IS_WOO21' => self::$sm_is_woo21, 'SM_BETA_PRO' => SMBETAPRO, 'record_per_page' => $record_per_page, 'sm_admin_email' => get_option('admin_email'), 'batch_background_process' => $batch_background_process, 'background_process_name' => $background_process_name, 'updated_sucessfull' => $successful, 'updated_msg' => $this->update_msg.' more', 'success_msg' => $this->success_msg, 'lite_dashboards' => json_encode($lite_dashboards) ) );
				...
				<div style="margin-bottom:1em;">
					...
					<span style="float: right; line-height: 2.5em;"> <?php
			            ...
			            if ( !empty( $_GET['page'] ) && $_GET['page'] == "smart-manager" && !( !empty( $_GET['sm_old'] ) && ( 'woo' === $_GET['sm_old'] || 'wpsc' === $_GET['sm_old'] ) ) ) {
			            	...
			            	if ( SMBETAPRO === true ) {
			            		$sm_old_link = '<span style="background-color: #ecddef;padding: 0.5em 0.5em 0.5em 0.5em;margin: 1.2em;border: 1px solid #4e4e8a;margin-top: 2em;">
			            	...
			            	if( !empty( $sm_old ) ) {
			            		$sm_old_link .= '<a href="'. admin_url('admin.php?page='. $_GET['page'] .'&sm_old='. $sm_old) .'" title="'. __( 'Switch back to Smart Manager Old', self::$text_domain ) .'"> ' . __( 'Switch back to Smart Manager Old', self::$text_domain ) .'</a> | ';	
			            	}
	                    ...
	                    if ( SMBETAPRO === true ) {
							...
							//			printf ( __ ( '%1s%2s%3s<a href="%4s" target=_storeapps>Docs</a>' , self::$text_domain), $before_plug_page, $plug_page, $after_plug_page, "https://www.storeapps.org/support/documentation/" );
								printf ( __ ( '%1s%2s<a href="%3s" target="_blank">Docs</a>' , self::$text_domain), $sm_old_link, $before_plug_page, "https://www.storeapps.org/knowledgebase_category/smart-manager/?utm_source=sm&utm_medium=sm_beta_grid&utm_campaign=view_docs" );
								?>
								</span><?php
